-- Archivo: Scripts_BD\001_init_database.sql
-- Script SQL de creacion inicial de tablas y datos estructurales de la base de datos.

# CREACION BASE DATOS

DROP DATABASE IF EXISTS cineroadmap;
CREATE DATABASE cineroadmap
...
# Retos iniciales disponibles para crear dinamica diaria, semanal y tematica.
...
CREATE INDEX idx_valoraciones_usuario_fecha ON valoraciones(usuario_id, fecha_registro);
CREATE INDEX idx_logros_tipo_valor ON logros(tipo_requisito, valor_requisito);
CREATE INDEX idx_usuario_retos_estado ON usuario_retos(usuario_id, estado);
